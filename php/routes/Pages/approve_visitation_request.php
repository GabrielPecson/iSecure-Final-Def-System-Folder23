    ...
    if (!empty($id)) {
        try {
            ...
            if ($request) {
                ...
                // Set session notification for pendings.php
                $_SESSION['notification_message'] = "Visitation request for " . htmlspecialchars($visitor_name) . " has been " . $action . "d.";
                $_SESSION['notification_type'] = ($action === 'approve') ? 'success' : 'error';
            }

            header('Location: pendings.php');
            exit;
        } catch (Exception $e) {
            $_SESSION['notification_message'] = 'Error processing request: ' . $e->getMessage();
