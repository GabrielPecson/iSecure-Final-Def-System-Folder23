...
def authenticate_face_endpoint():
    try:
        ...
        # Fetch the visitor's selfie path from the database
        ...
        # Construct absolute path to selfie
        project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
        ...
        # Verify the captured face against the visitor's selfie
        try:
            from deepface import DeepFace
            ...
            if "Face could not be detected" in str(e):
                ...
                return jsonify({"success": False, "message": f"Face authentication error: {str(e)}"})
...
def register_from_selfie_endpoint():
    try:
        ...
        from app.services.face_recog.face_authenticator import register_visitor
        success, message = register_visitor(visitor_name, absolute_selfie_path) # Use visitor_name for registration
...
if __name__ == '__main__':
    ...
    # Run Flask without SSL for localhost
